...
struct DFLadder : CubicDMFTSC<Solver,D,ksize> {
    ...
    GKType SigmaD;
protected:
    std::array<KMesh::point, D> _shift_point(const std::array<KMesh::point, D> &in, const std::array<KMesh::point, D> &shift) const;
...
struct DFLadder2d : DFLadder<Solver, 2, ksize>
{
    ...
    using DFLadder<Solver,2,ksize>::_qGrid;
    using DFLadder<Solver,2,ksize>::_shift_point;
    
    using DFLadder<Solver,2,ksize>::GD0;
    using DFLadder<Solver,2,ksize>::SigmaD;

    DFLadder2d(const Solver &S, const FMatsubaraGrid& fGrid, const BMatsubaraGrid& bGrid, RealType t):DFLadder<Solver,2,ksize>(S,fGrid,bGrid,t){};
