...
int main()
{
    ...
    CubicDMFTSC<FKImpuritySolver, 2, KPOINTS> SC(Solver, t);
    ...
    DFLadder<FKImpuritySolver, 2, KPOINTS> SC_DF(Solver, gridF, gridB, qGrids, t);
    SC_DF._n_GD_iter = 0;
    SC_DF();
    ...
    for (auto iW : gridB.getVals()) {
        ...
        auto chiD0 = SC_DF.getBubble(std::forward_as_tuple(iW,kGrid[0],kGrid[0]));
        auto chiDPI = SC_DF.getBubble(std::forward_as_tuple(iW,kGrid[KPOINTS/2],kGrid[KPOINTS/2]));
